...
function getApartmentById(id){
	...
	$.ajax({
		...
		success: function(apartman) {
			...
			//console.log(new Date(apartman.availableDates[0]));
			
			
			
			
			
			for(let i = 0; i<apartman.availableDates.length; i++){
				dates.push(formatDate(apartman.availableDates[i]));
			}
			

			
            datelist = dates; // populate the array

    	    $("#datepicker").datepicker({
    	        beforeShowDay: function(d) {
    	            
    	            var dt = formatDate(d);
    	            if ($.inArray(dt, dates) >= 0) {
    	            	console.log("true");
    	                return [true, ""];
